...
namespace GeoStar {
  ...
  void Shape::getGeometries()
  {
    ...
    // this gets every row....
    SQLite::Statement   query(*db, "SELECT geometry, PK from shape");
    
    // get contents of rows 1-at-a-time:
    while(query.executeStep()){
      SQLite::Column col = query.getColumn(0);
      int PK_id = query.getColumn(1);
      std::cout<<"Shape::getGeometries: "<<SpatiaLite::GeometryCollection(col).toWKTString()<<" PK="<<PK_id<<std::endl;
    }//endwhile
...
// overload function
// return shape that contains the results
void Shape::getBoundaryReturnShape(Shape * res_shape) {
      
      SQLite::Statement query (*db, "select PK, Boundary(CastToSingle(geometry)) from shape");

      while (query.executeStep()) {
	int PK = query.getColumn(0);
	std::string PK_str = std::to_string(PK);
	SQLite::Column col = query.getColumn(1);
	std::string geoText = SpatiaLite::GeometryCollection(col).toWKTString();
	std::string srid_string = std::to_string(getSRID());

	std::string q = (((((std::string("INSERT INTO shape (PK, geometry) VALUES ("+PK_str)+",CastToGeometryCollection(GeomFromText('")+geoText)+"', ")+srid_string)+")))");
	SQLite::Statement   query2(*(res_shape->getDbHandle()), q);
        query2.exec();
    }
}

void Shape::getEnvelopeReturnShape(Shape * res_shape) {
	...
	std::string srid_string = std::to_string(getSRID());
	std::string q = (((((std::string("INSERT INTO shape (PK, geometry) VALUES ("+PK_str)+",CastToGeometryCollection(GeomFromText('")+geoText)+"', ")+srid_string)+")))");
	std::cout << geoText<<std::endl;
	SQLite::Statement   query2(*(res_shape->getDbHandle()), q);
        query2.exec();
    }
}

void Shape::getExpandReturnShape(Shape * res_shape, int amount) {
      std::string amount_str = std::to_string(amount);
      SQLite::Statement query (*db, "select PK, ST_Expand(CastToSingle(geometry), " + amount_str + ") from shape");
...
void Shape::getCentroidReturnShape(Shape * res_shape) {
      SQLite::Statement query (*db, "select PK, Centroid(CastToSingle(geometry)) from shape");
...
void Shape::getSimplifyReturnShape(Shape * res_shape) {
      SQLite::Statement query (*db, "select PK, Simplify(CastToSingle(geometry), 0.5) from shape where GeometryType(CastToSingle(geometry)) = 'LINESTRING' OR GeometryType(CastToSingle(geometry)) = 'RING'");
...
// same query function lists as above
	...
	std::string PK_string = std::to_string(PK_id);
	SQLite::Statement query (*db, "select CastToSingle(geometry) from shape WHERE PK = " + PK_string);
	while (query.executeStep())
	    return SpatiaLite::GeometryCollection(query.getColumn(0)).toWKTString();
}

    void Shape::deleteRow(int PK_id) {
	std::string PK_string = std::to_string(PK_id);
        SQLite::Statement query (*db, "delete from shape WHERE PK = " + PK_string);
        query.exec();
}
    ...
    void Shape::deletePointInPolygon(int PK_id, std::string point_to_delete) {
	std::string PK_string = std::to_string(PK_id);
	SQLite::Statement query (*db, "select CastToSingle(geometry) from shape WHERE PK = "+ 
	PK_string + " AND GeometryType(CastToSingle(geometry)) = 'POLYGON'");
	while (query.executeStep()) {
	    std::string oldGeoText = SpatiaLite::GeometryCollection(query.getColumn(0)).toWKTString();
	    Shape::deleteRow(PK_id);
	    size_t idx = oldGeoText.find(point_to_delete);
    ...
    void Shape::deleteRingInPolygon(int PK_id, std::string ring_to_delete) {
	std::string PK_string = std::to_string(PK_id);
	SQLite::Statement query (*db, "select CastToSingle(geometry) from shape WHERE PK = "+ 
	PK_string + " AND GeometryType(CastToSingle(geometry)) = 'POLYGON'");
	while (query.executeStep()) {
	    std::string oldGeoText = SpatiaLite::GeometryCollection(query.getColumn(0)).toWKTString();
	    Shape::deleteRow(PK_id);
	    size_t idx = oldGeoText.find(ring_to_delete);
	    if (idx < 0 || idx >= oldGeoText.size()) return;
	    std::string newGeoText = oldGeoText.substr(0, idx) + oldGeoText.substr(idx + ring_to_delete.size());
	    std::string srid_string = std::to_string(getSRID());
	    std::string q = ((((std::string("INSERT INTO shape (geometry) VALUES (CastToGeometryCollection(GeomFromText('")+newGeoText)+"', ")+srid_string)+")))");
	    SQLite::Statement   query2(*db, q);
            query2.exec();
	...
	void Shape::makecopy(int PK_id) {
	std::string PK_string = std::to_string(PK_id);
	SQLite::Statement query (*db, "select CastToSingle(geometry) from shape WHERE PK = "+ PK_string);
	while (query.executeStep()) {
		std::string text = SpatiaLite::GeometryCollection(query.getColumn(0)).toWKTString();
	    std::string srid_string = std::to_string(getSRID());
	    std::string q = ((((std::string("INSERT INTO shape (geometry) VALUES (CastToGeometryCollection(GeomFromText('")+text)+"', ")+srid_string)+")))");
	    SQLite::Statement   query2(*db, q);
        query2.exec();
	    return;
	}
	...
	std::cout<<"begin write to file:"<<geometry<<std::endl;
	myfile<<geometry<<'\n';
}
